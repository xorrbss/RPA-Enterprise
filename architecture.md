...
| IREL evaluator | ir-expression | (D1 추가 산출 권장) | 자체 파서(no eval) |
...
**D1**(거의 완료: `codegen/` 산출) → **D2** transition 런타임 + Worker 골격(transitions.ts 연결) → **D3** Executor 골격(**Stagehand v3 CDP 기반 Utility/PageStateResolver 먼저**, Stagehand act/observe/extract 후행) → **D4** Control-plane API(openapi→Fastify, RBAC/RLS) → **D5** LLM Gateway(Codex SSE) → **D6** Pipeline/Sink + outbox 소비 → **D7** Frontend(콘솔). 이후 P2 RLS 활성화 → P3 Remote Agent(event bridge).
