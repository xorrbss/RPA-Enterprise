...
### 9.1 구현 순서
1. **UtilityExecutor(CDP)** + **PageStateResolver** — Stagehand v3 CDP page 기반(LLM 미사용). 비브라우저(`api_call`/`file`/`shell`)는 별 모듈(Node). **[완료 2026-06-14]** `app/src/executor/`(cdp-session.ts 포트+Stagehand 어댑터 · page-state-resolver.ts · utility-executor.ts), tsc strict + dry-run 통합 green.
...
- **[PoC 완료 2026-06-14 — 10/10 PASS]** 실측: `@browserbasehq/stagehand@3.5.0` + 로컬 Chrome CDP, `act`/LLM 미사용. 증거·재현: `app/poc/d3-stagehand/`(`D3-POC-EVIDENCE.md`, `npm run poc`). raw CDP 보완(structuralHash·download·auth·flags·abort)은 전부 Stagehand 공개 `page.sendCDP()`(동일 세션, §9.5)로 충족 — **외부 드라이버 추가 불필요**. 발견: abort 시 in-flight `sendCDP`는 reject 없이 무응답이라 타임아웃으로 단절 판정 필요(`CDP_DISCONNECTED` 매핑).
...
**[충족 2026-06-14]** 3개 기준 모두 통과. `app/test/executor-dryrun.int.ts`(실 Stagehand v3 + 로컬 Chrome, `act` 미사용): navigate→resolve(flags·structuralHash)→`selectOnBranch` `on[]`→verify→download→pagination 루프 + `transitionRun` lifecycle(queued→claimed→running→completing→completed), DB/outbox/전송 미사용 dry-run 18/18 green. 실행 `npm --prefix app run test:executor`(로컬 Chrome 필요라 test:ci 미포함). dom 액션은 `EXECUTOR_CAPABILITY_MISMATCH`로 명시 거부(조용한 no-op 금지).
