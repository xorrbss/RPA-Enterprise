...
| 알림 발화 하드닝 (S4b) | **계약 변경**(DDL) | ① `ops_notification_attempts` 세대 유니크 인덱스+ON CONFLICT(멀티워커 airtight 멱등 — S4a 는 단일 poll SELECT-guard 로 커버) ② `source='session_expiry'` CHECK 확장(세션 만료를 attempt 파이프라인으로 발화) ③ 테넌트별 저장형 라우팅 테이블+관리 UI(멀티테넌트 셀프서비스). 전부 파일럿엔 불필요, 규모 확대 시 오너 결정 |
| 오프보딩 데이터 반출 | feature | runs/human_tasks/artifacts 일괄 export 계약 신설 필요. 심사 단골 질문이므로 v-next 우선 후보 |
