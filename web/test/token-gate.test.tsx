...
describe("TokenGate — 접속/온보딩 + 세션 만료", () => {
  test("토큰 없으면 접속 화면 + 발급처 안내(어디서 받는지)", () => {
    ...
    expect(screen.getByText(/관리자\/IT 담당자가 발급/)).toBeInTheDocument();
    expect(screen.queryByText("APP")).toBeNull();
  });

  test("빈 토큰 제출 → 인라인 피드백, 입장 안 됨(조용한 무시 금지)", () => {
    render(
      <TokenGate>
        <div>APP</div>
      </TokenGate>,
    ...
    expect(screen.getByText("토큰을 입력하세요.")).toBeInTheDocument();
    expect(screen.queryByText("APP")).toBeNull();
    expect(localStorage.getItem(KEY)).toBeNull();
  });

  test("유효 토큰 제출 → 입장 + localStorage 저장", () => {
    render(
      <TokenGate>
        <div>APP</div>
      </TokenGate>,
    );
    fireEvent.change(screen.getByLabelText("Bearer 토큰"), { target: { value: "eyJ.aaa.bbb" } });
    fireEvent.click(screen.getByRole("button", { name: "접속" }));
  ...
  test("세션 만료(notifyAuthExpired) → 토큰 제거 + 만료 안내 + 게이트 복귀(리로드 없이)", () => {
    ...
    expect(screen.queryByText("APP")).toBeNull();
    expect(screen.getByText(/세션이 만료되었거나 토큰이 유효하지 않습니다/)).toBeInTheDocument();
    expect(localStorage.getItem(KEY)).toBeNull();
  });
});
