...
export const ACTIONS = ["none", "observe", "act", "extract", "navigate"] as const;
...
export const DEFAULT_SCHEMA_REF = "수집데이터";
export const DEFAULT_URL_REF = "이동주소";

export type Rule = { when: string; target: string; priority: number };
export type Flow =
...
export const FLOW_LABELS: Record<Flow["kind"], string> = {
  terminal: "종료",
  next: "다음 단계로",
  on: "조건 분기",
  loop: "반복",
...
export function defaultExtractInstruction(schemaRef?: string): string {
  return `현재 페이지에서 ${dataNameForDisplay(schemaRef)} 내용을 추출하라.`;
}

export function defaultActionInstruction(action: Step["action"]): string {
...
export const SELECT = { padding: "4px 6px", fontSize: 13 } as const;
...
export function initialCounter(steps: readonly Step[]): number {
