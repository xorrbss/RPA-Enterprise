...
export function CaptureGuide({ site, onClose }: { site: SiteItem; onClose: () => void }): JSX.Element {
  ...
  const command = `$env:RPA_OPERATOR_TOKEN="<본인 접속 코드>"; npm --prefix app run session:capture-helper -- --api ${resolveApiBase()} --site ${site.site_profile_id}`;
    ...
    <ConfirmDialog title={`브라우저 로그인 세션 등록 — ${name}`} hideConfirm cancelLabel="닫기" onConfirm={onClose} onCancel={onClose}>
      <div style={{ display: "grid", gap: 12, fontSize: 13, lineHeight: 1.5 }}>
        ...
        <p style={{ margin: 0 }} className="subtle">
          사전 준비: 운영자 PC에 저장소가 체크아웃되어 있고 Node.js가 설치되어 있어야 합니다.
        </p>
        <p style={{ margin: 0 }}>
          IT 담당자에게 이 명령을 전달하세요. 등록 후 이 사이트는 “세션 등록됨”으로 표시됩니다.
        </p>
        <details className="developer-details">
          <summary>Windows PowerShell 실행 방법 보기</summary>
          <pre
            style={{
              margin: "8px 0 0",
              padding: "10px 12px",
