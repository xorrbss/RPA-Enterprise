...
export function PolicyReadout({ policy }: { policy: GatewayPolicy }): JSX.Element {
  return (
...
export function GatewayEditForm({ policy }: { policy: GatewayPolicy }): JSX.Element {
  ...
  const [fallback, setFallback] = useState(() => JSON.stringify(policy.fallback ?? null, null, 2));
  const [isDefault, setIsDefault] = useState(() => policy.is_default === true);
  const [jsonMode, setJsonMode] = useState(() => policy.capabilities?.jsonMode === true);
  const [vision, setVision] = useState(() => policy.capabilities?.vision === true);
  const [maxContextTokens, setMaxContextTokens] = useState(() => String(policy.capabilities?.maxContextTokens ?? 8000));
  const [maxInputTokens, setMaxInputTokens] = useState(() => String(policy.budget?.maxInputTokens ?? 1000));
  const [maxOutputTokens, setMaxOutputTokens] = useState(() => String(policy.budget?.maxOutputTokens ?? 1000));
  const [maxCost, setMaxCost] = useState(() => String(policy.budget?.maxCost ?? 1));
  const [fallbackModel, setFallbackModel] = useState(() => typeof policy.fallback?.model === "string" ? policy.fallback.model : "");
  const [advanced, setAdvanced] = useState(false);
  const [msg, setMsg] = useState<{ tone: "green" | "red"; text: string } | null>(null);

  const save = useMutation({
    mutationFn: (body: GatewayPolicyUpdate) => api.updateGatewayPolicy(policy.version as number, body, crypto.randomUUID()),
    onSuccess: () => {
  ...
  function onSubmit(e: React.FormEvent): void {
    ...
    const structured = applyStructuredPolicy(parsed.body, {
      jsonMode, vision, maxContextTokens, maxInputTokens, maxOutputTokens, maxCost, fallbackModel,
    });
    if (structured.kind === "error") {
      setMsg({ tone: "red", text: structured.message });
      return;
    }
    save.mutate({ model: policy.model, ...structured.body, is_default: isDefault });
  }
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: "grid", gap: 10, maxWidth: 640 }}>
      ...
      />
      <button className="btn" type="button" onClick={() => setAdvanced((v) => !v)}>
        {advanced ? "고급 JSON 닫기" : "고급 JSON 열기"}
      </button>
      {advanced && (
        <>
...
export function GatewayCreateForm(): JSX.Element {
  ...
  function onSubmit(e: React.FormEvent): void {
    ...
    create.mutate({ model: nextModel, ...structured.body, is_default: isDefault });
  }
...
function StructuredPolicyFields(props: {
  ...
  return (
    <div className="policy-fields">
      <label><span className="label">컨텍스트 한도</span><input value={props.maxContextTokens} onChange={(e) => props.setMaxContextTokens(e.target.value)} inputMode="numeric" /></label>
      <label><span className="label">입력 토큰 한도</span><input value={props.maxInputTokens} onChange={(e) => props.setMaxInputTokens(e.target.value)} inputMode="numeric" /></label>
      <label><span className="label">출력 토큰 한도</span><input value={props.maxOutputTokens} onChange={(e) => props.setMaxOutputTokens(e.target.value)} inputMode="numeric" /></label>
      <label><span className="label">비용 한도</span><input value={props.maxCost} onChange={(e) => props.setMaxCost(e.target.value)} inputMode="decimal" /></label>
      <label className="check-field"><input type="checkbox" checked={props.jsonMode} onChange={(e) => props.setJsonMode(e.target.checked)} /> JSON 모드 지원</label>
      <label className="check-field"><input type="checkbox" checked={props.vision} onChange={(e) => props.setVision(e.target.checked)} /> 비전 입력 지원</label>
      <label><span className="label">fallback 모델</span><input value={props.fallbackModel} onChange={(e) => props.setFallbackModel(e.target.value)} placeholder="선택: 예비 모델명" /></label>
    </div>
  );
}
...
function parseNonNegative(value: string, label: string): number | { error: string } {
  const n = Number(value);
  if (!Number.isFinite(n) || n < 0) return { error: `${label}은 0 이상의 숫자여야 합니다.` };
  return n;
}

function applyStructuredPolicy(
  body: Pick<GatewayPolicyUpdate, "capabilities" | "budget" | "fallback_config">,
  fields: {
    jsonMode: boolean;
    vision: boolean;
    maxContextTokens: string;
  ...
  | { kind: "ok"; body: Pick<GatewayPolicyUpdate, "capabilities" | "budget" | "fallback_config"> }
  | { kind: "error"; message: string } {
  const maxContextTokens = parseNonNegative(fields.maxContextTokens, "컨텍스트 한도");
  const maxInputTokens = parseNonNegative(fields.maxInputTokens, "입력 토큰 한도");
  const maxOutputTokens = parseNonNegative(fields.maxOutputTokens, "출력 토큰 한도");
  const maxCost = parseNonNegative(fields.maxCost, "비용 한도");
  if (typeof maxContextTokens !== "number") return { kind: "error", message: maxContextTokens.error };
  if (typeof maxInputTokens !== "number") return { kind: "error", message: maxInputTokens.error };
  if (typeof maxOutputTokens !== "number") return { kind: "error", message: maxOutputTokens.error };
  if (typeof maxCost !== "number") return { kind: "error", message: maxCost.error };
  const fallbackName = fields.fallbackModel.trim();
  const fallback_config = fallbackName.length > 0
    ? { ...(body.fallback_config ?? {}), model: fallbackName }
    : body.fallback_config ?? null;
  return {
    kind: "ok",
    body: {
      capabilities: { ...body.capabilities, maxContextTokens, jsonMode: fields.jsonMode, vision: fields.vision },
