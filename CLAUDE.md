...
3. `codegen/` — **D1 outputs generated from the contracts** (TS types, ajv validators, static validation, event payload registry, transition functions, error middleware, OpenAPI/AsyncAPI, fixtures). This is the one part that **builds/runs**: it has its own `package.json`/`tsconfig.json` — `npm install --prefix codegen` then `npm --prefix codegen run typecheck` (tsc strict) / `run fixtures` (validators 36, static validation 17, transition 70 cases). `node_modules/` is gitignored. Regenerate from contracts when contracts change; do not hand-edit as a source of truth.
...
| `db/migration_core_entities.sql` | Core entity DDL (18 tables): runs/run_steps/workitems/human_tasks/scenarios/scenario_versions/artifacts/events_outbox/dead_letter/stagehand_calls/action_plan_cache/site_profiles/site_profile_approvals/browser_identities/network_policies/gateway_policies/control_plane_idempotency_keys/workers. Status CHECKs mirror `state-machine-types.ts`. Adds tenant composite FKs/RLS hardening to concurrency-migration tables via ALTER (apply order: concurrency → core). | `state-machine-types.ts`, `core-types.ts`, `event-envelope.schema.json`, `migration_concurrency_idempotency.sql` |
...
Do not invent event-specific payload body fields. Product Open v1 fixes the `event_type` ↔ `payload_schema_ref` mapping and closed-empty per-event payload body schemas; adding body fields requires a versioned schema change. Codex SSE structured-output streaming scope/abort behavior is handled by capability gates and safe fallback, but any implementation depending on live provider behavior must verify it rather than guessing.
