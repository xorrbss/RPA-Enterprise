...
- [ ] **(채택: HashiCorp Vault — D8-A14)** 실제 Vault mount/path (평문 금지, 실제 백엔드 식별):
...
- [ ] **(채택: S3/S3-호환 — D8-A14)** 실제 S3 bucket + SecretRef-backed `credentialRef` (평문 금지, `real_object_store` `backendAlias`):
