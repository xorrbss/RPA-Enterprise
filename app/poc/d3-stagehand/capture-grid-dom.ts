  ...
  try {
    ...
    const dom = await page.evaluate(() => {
      ...
      grid?.querySelectorAll("[class]").forEach((el) => {
        // getAttribute('class') 는 HTML/SVG 모두 클래스 문자열을 준다(el.className 은 SVG 에서 SVGAnimatedString → '[object …]' 오염).
        (el.getAttribute("class") || "").split(/\s+/).forEach((c) => { if (c) classCount[c] = (classCount[c] || 0) + 1; });
      });
