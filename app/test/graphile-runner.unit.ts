...
check(
  "trigger_fire routes to control task",
...
check(
  "artifact_orphan routes to lifecycle task (BYPASSRLS global object_ref scan)",
  runtimeJobTaskIdentifier({ kind: "artifact_orphan" } as never) === RUNTIME_LIFECYCLE_JOB_TASK,
);
