 ...
 *   start_url 을 열고, 브라우저 녹화 helper 코어(defaultLaunchBrowser/sanitizePageEvent)를 그대로 재사용해 click/input/
 ...
 *
 * 보안: 자격증명 입력 등 민감 타깃은 helper 코어의 sanitizePageEvent 가 drop(기존 prod 경로와 동일).
...
import {
  ...
  sanitizePageEvent,
  type BrowserRecordingLaunchInput,
...
export interface RecordingLoopDeps {
  readonly findChrome?: () => string | null;
...
export async function startRecordingLoop(pool: Pool, tenantId: string, intervalMs = 2000, deps: RecordingLoopDeps = {}): Promise<RecordingLoop | null> {
  const chrome = (deps.findChrome ?? findChrome)();
  const launchBrowser = deps.launchBrowser ?? defaultLaunchBrowser;
  const newId = deps.newId ?? randomUUID;
  const log = deps.log ?? console.log;
  if (chrome === null) {
    log("recording-loop: Chrome 미발견 → 브라우저 녹화 비활성(CHROME_PATH 설정 시 활성).");
    return null;
  }
  log("recording-loop: 활성 — 콘솔 '브라우저 녹화로 만들기'의 녹화 시작(status=recording)을 폴링해 headful Chrome 을 띄운다.");
  ...
  const insertEvent = async (sessionId: string, ev: SanitizedRecordingEvent): Promise<void> => {
    ...
    await withTenantTx(pool, tenantId, async (c) => {
      await c.query(
        ...
        [newId(), tenantId, sessionId, seq, ev.event_type, ev.selector ?? null, ev.label ?? null, ev.url ?? null, ev.value_preview ?? null],
      );
      // 콘솔이 표시하는 진행 카운트 동기화.
  ...
  const tick = async (): Promise<void> => {
    ...
    try {
      ...
      const handle = await launchBrowser({
        ...
        log: (m) => log(`recording-loop ${sessionId.slice(0, 8)}: ${m}`),
      });
      openHandles.set(sessionId, handle);
      log(`recording-loop: ${sessionId.slice(0, 8)} headful Chrome 오픈(${row.start_url}) — 화면에서 클릭·입력하면 동작이 콘솔 목록에 쌓입니다. 창을 닫으면 캡처 종료.`);
      void handle.waitUntilClosed().then(() => {
        openHandles.delete(sessionId);
        log(`recording-loop: ${sessionId.slice(0, 8)} Chrome 닫힘 — 콘솔에서 동작을 검토하고 '녹화 완료'로 봇 초안을 만드세요.`);
      });
    } catch (e) {
