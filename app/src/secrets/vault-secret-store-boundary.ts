 ...
 * `api`→resume_token_hmac(검증); signed_command 는 `SecretAccessRequest.purpose` enum 밖이라 여기 미포함
 * (해당 namespace 는 SignedCommandRegistry 경계 책임 — 본 boundary 의 purpose 집합과 분리).
 */
const RESOLVE_MATRIX: Readonly<Record<RuntimeIdentity, readonly ResolvePurpose[]>> = {
  api: ["resume_token_hmac"],
  "runtime-worker": ["resume_token_hmac", "executor"],
