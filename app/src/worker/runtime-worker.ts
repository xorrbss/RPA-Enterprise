...
// TODO: [BLOCKED]
//   violated: CLAUDE.md #7 (파일 500라인 제한 — 현재 1189)
//   reason: 잔여는 run-claim/run-resume 코어가 browser-lease 획득·worker-circuit·run-state 전이를 동일 tx
//     안에서 엮은 라이브 엔진 핵심이다. 의존은 DAG(run-lifecycle → lease/circuit/init leaf, 역호출 없음 — 실측
//     확인)이라 협력객체 분리 자체는 가능하나, 215/252줄짜리 handleRunClaim/handleRunResume 를 deps-threading
//     협력객체로 옮기는 것은 동작보존 sibling 추출 범위를 넘는 아키텍처 리팩터이며 claim/resume 회귀 위험이
//     최고다. Phase1/2(PR #190~#202)로 2690→1189 까지는 안전 축소 완료.
//   required_change: run-drive 협력객체(handleRunClaim/Resume/Abort) + worker-run-support 협력객체
//     (acquireBrowserLease/checkWorkerCircuit/loadExpectedRun/recordWorkerInit*) 2단 분리 — 전용 세션에서
//     적대검증 + 전체 runtime-worker int 스위트(claim/resume/resume-drive/abort/drive/circuit)로 회귀 차단 후 진행.
