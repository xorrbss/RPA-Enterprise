...
const SYSTEM_PROMPT = [
  ...
  "For extraction nodes, return data shaped as { summary: string, rows: object[] }; never fabricate values that are not visible on the page.",
  "For multi-page collection, model a bounded loop driven by a max_pages param rather than unbounded or imagined repetition.",
  "Declare the run target as { site_profile_id, browser_identity_id, network_policy_id }; if the approved target is unknown, omit target and add a blocker instead of guessing ids.",
