...
import {
  executeAssertAbsent,
  executeClickText,
  executeDeterministicClick,
  executeRichBodyFill,
} from "./stagehand-dom-executor-deterministic";
...
export class StagehandDomExecutor implements ExecutorPlugin {
  ...
  private async executeAct(
    ...
    if (a.clickSelector !== undefined) {
      return executeDeterministicClick(stepId, a.clickSelector, a.sideEffect, ctx, session, before, startedAt);
    }
    // 결정형 텍스트 클릭(click_text): 보이는 텍스트 포함 첫 상호작용 요소를 브라우저 JS 로 찾아 클릭(LLM·CSS 미경유).
    if (a.clickText !== undefined) {
      return executeClickText(stepId, a.clickText, a.sideEffect, ctx, session, before, startedAt);
    }
    // 결정형 부재 단언(assert_absent): 셀렉터 소멸까지 settle — 비가역 커밋 효과 witness(잔존 시 loud).
    if (a.assertAbsent !== undefined) {
      return executeAssertAbsent(stepId, a.assertAbsent, a.sideEffect, ctx, session, before, startedAt);
    }
    ...
    if (a.richBodyFrame !== undefined) {
      return executeRichBodyFill(stepId, a.richBodyFrame, a.value, a.sideEffect, ctx, session, before, startedAt);
    }
  ...
  // 결정형 act 핸들러(click_selector/click_text/rich_body_frame/assert_absent)는
  // stagehand-dom-executor-deterministic.ts(의미 단위 분리, CLAUDE.md #7).
