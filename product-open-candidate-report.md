...
evidence is `main` `Contract Gates` run `27850697692` on commit
`b3232d92495a0d774660bff1141177d8bf2613f0`, superseding the earlier D4.4
and Phase 7 remote evidence packets for current-main release judgment.
This current merged delta exposes generation-level result artifact discovery for
natural-language save-and-run flows; any remaining active blocker is listed in
...
- Current `main` merged repo-controlled product-code evidence:
  `https://github.com/xorrbss/RPA-Enterprise/actions/runs/27850697692`
  (`Contract Gates` success on commit `b3232d92495a0d774660bff1141177d8bf2613f0`).
- Current `main` required job URLs:
  `secret-scan`: `https://github.com/xorrbss/RPA-Enterprise/actions/runs/27850697692/job/82429028841`
  `PostgreSQL 15 migration smoke`: `https://github.com/xorrbss/RPA-Enterprise/actions/runs/27850697692/job/82429028756`
  `App runtime typecheck and tests`: `https://github.com/xorrbss/RPA-Enterprise/actions/runs/27850697692/job/82429028745`
- Phase 7 `main` runtime-delta `Contract Gates` attempt:
  ...
  `27850697692` (commit `b3232d92495a0d774660bff1141177d8bf2613f0`) is the
...
- Repo-controlled Product Open Candidate gap: none remain for the tagged
  ...
  `27850697692` on commit `b3232d92495a0d774660bff1141177d8bf2613f0`; this closes only the
...
- Resolved (remote evidence): Runtime execution staging gates now have current remote `main` CI evidence. GitHub Actions hosted-runner execution is restored (the account payment/spending-limit blocker no longer applies — `main` `Contract Gates` runs start and succeed). The Phase 7 runtime delta is merged on `main` (`executor-step-orchestrator.ts` / `executor-completion-coordinator.ts` / `executor-invocation-recorder.ts`), and `main` `Contract Gates` run `27850697692` (commit `b3232d92495a0d774660bff1141177d8bf2613f0`) is `success` with the required job URLs — `App runtime typecheck and tests` (https://github.com/xorrbss/RPA-Enterprise/actions/runs/27850697692/job/82429028745), `Secret scan` (https://github.com/xorrbss/RPA-Enterprise/actions/runs/27850697692/job/82429028841), `PostgreSQL 15 migration smoke` (https://github.com/xorrbss/RPA-Enterprise/actions/runs/27850697692/job/82429028756). The `App runtime typecheck and tests` job runs `test:int` under a non-`SUPERUSER`/non-`BYPASSRLS` PostgreSQL 15 role, proving tenant boundary / RBAC/redaction / no `BYPASSRLS` / no silent false-unknown. Former Required decision: restore GitHub Actions hosted-runner execution, rerun `Contract Gates` on the Phase 7 `main` head, and provide the required PR/main job URLs.
