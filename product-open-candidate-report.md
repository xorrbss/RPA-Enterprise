...
evidence is the latest `main` `Contract Gates` run `27848073200` on commit
`fed079efaca0cc54825530041e909b8d0220e2c8`, superseding the earlier D4.4
...
- Current `main` merged repo-controlled evidence:
  `https://github.com/xorrbss/RPA-Enterprise/actions/runs/27848073200`
  (`Contract Gates` success on commit `fed079efaca0cc54825530041e909b8d0220e2c8`).
- Current `main` required job URLs:
  `secret-scan`: `https://github.com/xorrbss/RPA-Enterprise/actions/runs/27848073200/job/82421409307`
  `PostgreSQL 15 migration smoke`: `https://github.com/xorrbss/RPA-Enterprise/actions/runs/27848073200/job/82421409236`
  `App runtime typecheck and tests`: `https://github.com/xorrbss/RPA-Enterprise/actions/runs/27848073200/job/82421409297`
- Phase 7 `main` runtime-delta `Contract Gates` attempt:
  ...
  `27848073200` (commit `fed079efaca0cc54825530041e909b8d0220e2c8`) is the
...
- Repo-controlled Product Open Candidate gap: none remain for the tagged
  ...
  `27848073200` on commit `fed079efaca0cc54825530041e909b8d0220e2c8`; this closes only the
...
- Resolved (remote evidence): Runtime execution staging gates now have current remote `main` CI evidence. GitHub Actions hosted-runner execution is restored (the account payment/spending-limit blocker no longer applies — `main` `Contract Gates` runs start and succeed). The Phase 7 runtime delta is merged on `main` (`executor-step-orchestrator.ts` / `executor-completion-coordinator.ts` / `executor-invocation-recorder.ts`), and the latest `main` `Contract Gates` run `27848073200` (commit `fed079efaca0cc54825530041e909b8d0220e2c8`) is `success` with the required job URLs — `App runtime typecheck and tests` (https://github.com/xorrbss/RPA-Enterprise/actions/runs/27848073200/job/82421409297), `Secret scan` (https://github.com/xorrbss/RPA-Enterprise/actions/runs/27848073200/job/82421409307), `PostgreSQL 15 migration smoke` (https://github.com/xorrbss/RPA-Enterprise/actions/runs/27848073200/job/82421409236). The `App runtime typecheck and tests` job runs `test:int` under a non-`SUPERUSER`/non-`BYPASSRLS` PostgreSQL 15 role, proving tenant boundary / RBAC/redaction / no `BYPASSRLS` / no silent false-unknown. Former Required decision: restore GitHub Actions hosted-runner execution, rerun `Contract Gates` on the Phase 7 `main` head, and provide the required PR/main job URLs.
