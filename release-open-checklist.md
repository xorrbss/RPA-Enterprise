...
- [ ] D4.4 durable security audit writer boundary is named and evidenced. Required decision: define the repo-owned runtime/control-plane boundary that appends immutable audit rows for security boundary decisions, including artifact read, SecretStore resolve, connector enable/install, domain/prompt policy blocks, and any BYPASSRLS use; staging evidence must prove each covered path appends safely or is explicitly scoped out before executable staging readiness is claimed.
