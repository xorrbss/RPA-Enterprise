...
표준 노드 출력 필드(참조 가능): `row_count`(int), `status`(string), `extracted_ref`(string), `tier`(string, fallback 시), `http_status`(int), `http_ok`(boolean), `decision`(string, **@human_task 노드만**), `correction.<key>`(scalar, **@human_task 노드만**).
...
- `http_status`/`http_ok` ← **api_call 액션** StepResult의 HTTP 응답 상태. api_call이 아니거나 응답 스냅샷이 없으면 미투영.
- `decision`/`correction.<key>` ← **@human_task 노드**의 사람 판정 출력(`reserved-handlers.md` @human_task). 노드는 suspend→resolve→resume로 완료되며 그 출력이 판정이다. `decision`(string) = `human_tasks.result.decision`(닫힌 enum `approve`/`reject`/`correct`/`retry`, api-surface `RESOLUTION_DECISIONS` SSoT), `correction.<key>`(scalar) = `human_tasks.result.corrections.<key>`(business_form_v1 `fields[].key` 대응). 출처 노드 = `@human_task`를 **선언한 소유 노드 id**(`human_tasks.node_id`; `return_node` 아님). **주입 = 재개 시 런타임이 `human_tasks.result`를 re-SELECT**(resume token 미적재 — 서버 권위·RBAC+RLS 뒤·resolve 후 불변·idempotent; reserved-handlers.md "복귀(resume)"). @human_task 아닌 노드/미해소 task면 미투영(참조 시 `IREL_RUNTIME_MISSING` — 조용한 false 금지). 참조 가능 필드 한정은 `ir-static-validation.md` V9(@human_task 노드 게이트), 미해소 enum 분기 검증은 V13.
