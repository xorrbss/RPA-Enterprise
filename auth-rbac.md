...
export type Role =
  | "viewer"     // 조회 전용
  | "operator"   // 운영 실행/복구 (W10 manual_replay·run abort)
...
| `operator` | run `abort`(R6), DLQ `manual_replay`(W10 `operatorAuthorized`), sink DLQ replay, human_task 인박스 assign/start. 조회 포함. | state-machine W10 "운영자 재처리 권한", R6, error-catalog `DEAD_LETTER`/`SINK_DELIVERY_FAILED` operatorAction |
...
- **경계 강제**: 모든 요청 경계(제어평면 API 진입, queue job 소비, 이벤트 발행)에서 인증 컨텍스트의 tenant_id를 추출해 트랜잭션 세션에 바인딩(§4 `SET LOCAL`)한다. 클레임에 tenant_id가 없거나 다중 모호 → **요청 거부**(`AUTHZ_FORBIDDEN`), 통과 금지(조용한 false 금지).
...
신규 코드 `AUTHZ_FORBIDDEN`는 `error-catalog.ts`에 추가 필요(아래 제안 메타):
```ts
