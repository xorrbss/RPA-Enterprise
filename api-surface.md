...
| GET | `/v1/ops/production-readiness` | `ops_alert.read` 권한. controlled-prod 열림 여부를 runtime 증거와 owner/platform 증거 원장으로 분리해 계산. DB migration ledger, Graphile queue, browser pool HA, expired lease, stale run, audit verifier freshness는 현재 DB에서 판정하고 external alert delivery/PITR restore drill/SLO on-call sign-off/support training completion/observability telemetry wiring은 `production_readiness_evidence` 최신 유효 증거가 없으면 `deferred`로 표면화 | 200 + `ProductionReadiness`(`status`, `summary.controlled_prod_ready`, `gates[]`, `signals`). `deferred`는 성공이 아니며 `controlled_prod_ready=false`로 유지. 최신 증거가 `failed`면 해당 gate는 `blocked` | `AUTHZ_FORBIDDEN`(403) |
...
AI runtime enforcement(v1)은 evidence registry와 분리한다. Required decision: tenant/customer AI policy must choose enforcement mode (`observe`, `warn`, `block`), subject mapping, grace period, and emergency override owner before `/v1/runs` or prompt/model rollout paths may fail closed on missing or expired AI governance evidence.
