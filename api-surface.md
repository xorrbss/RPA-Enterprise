...
| Method | Path | 요청 | 응답 | 오류 |
|---|---|---|---|---|
| GET | `/v1/reports/automation-performance` | 쿼리: `?month=YYYY-MM`(기본 Asia/Seoul 현재 월). `run.read` 권한 | 200 + `{ month, timezone, period_start, period_end, summary, failure_top, trends, by_workflow }`. `summary`는 `total_runs`, `completed`, `failed_business`, `failed_system`, `success_rate`, `rerun_count`, `reprocessing_rate`, `estimated_hours_saved`, `estimated_value`, `gateway_cost`. `failure_top`은 실패 code Top 5. `trends`는 일별 run/실패/재처리/비용 추세. `by_workflow`는 scenario별 run/성공률/재처리율/ROI/비용 | `IR_SCHEMA_INVALID`(422), `AUTHZ_FORBIDDEN`(403) |
| GET | `/v1/reports/automation-performance/export` | 쿼리: `?month=YYYY-MM&format=csv\|xlsx\|poc_markdown`. `format` 생략 시 csv. `run.read` 권한 | csv는 `text/csv; charset=utf-8` + summary/failure/trends/workflow 섹션. xlsx는 `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` + Summary/Failure Top N/Daily Trends/Workflow ROI 시트. `poc_markdown`은 `text/markdown; charset=utf-8` + month, summary metrics, failure Top N, daily trends, workflow ROI/cost, decision guide를 포함하는 PoC 보고서 템플릿. CSV/XLSX 문자열 셀은 spreadsheet formula injection 방어를 적용하고, Markdown 문자열 셀은 formula 선두 문자, Markdown 링크, HTML 태그 해석을 방어적으로 escape한다. Secret/token/password/resolved secret material은 포함하지 않는다 | `IR_SCHEMA_INVALID`(422), `AUTHZ_FORBIDDEN`(403) |
...
| POST | `/v1/scim/principals` | `Idempotency-Key`. body `{ idp_provider, external_id, sub, display_name, email?, active?, roles }`. `scim.sync` 권한(admin) | 200 + `{ principal_id, sub, active, roles }`. SCIM principal upsert, active=true이면 active SCIM role set 동기화, active=false이면 해당 provider principal의 active SCIM assignments revoke. `scim.sync` 감사 기록 | `IR_SCHEMA_INVALID`, `AUTHZ_FORBIDDEN` |
...
- Role assignment v1: `principal_role_assignments`는 tenant-wide assignment ledger다. 콘솔/API grant는 `source='manual'`만 생성하고, SCIM sync endpoint는 `source='scim'`, `external_id`, `idp_provider`, `lifecycle_source='scim'` assignment를 생성/회수한다. effective roles는 token role과 active manual/SCIM assignment의 합집합이다. 자기 자신에게 `admin` 부여, 자기 자신의 마지막 `rbac.grant` 근거 회수, 만료된 `expires_at`, 중복 active `(tenant_id, principal_sub, role)`은 모두 거부한다. manual revoke API는 `source='manual'`만 회수하며, IdP/JWT claim role과 SCIM-managed assignment는 본 API로 회수할 수 없다. SCIM provider 등록, inbound auth/schema version, sub/external identity conflict rule은 별도 hardening 결정으로 남긴다.
