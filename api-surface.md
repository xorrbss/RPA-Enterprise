...
| POST | `/v1/sites/{site_profile_id}/recordings/{recording_session_id}/complete` | `Idempotency-Key`. 수정 권한(`site.update`) 필요 | 200 + recording session `status=completed`, `draft_ir`, `validation_report` 포함. `validation_report.stages[]`는 `well_formed/runnable/operable/prod_ready`를 분리하며, `completed`는 draft 생성 완료이지 실행/운영/prod 준비 성공이 아니다. draft_ir는 `navigate` + deterministic `act.args.*_selector` 중심의 초안이며, 서버는 같은 compile pipeline으로 정적 검증 리포트를 저장한다 | `AUTHZ_FORBIDDEN`(403), `RESOURCE_NOT_FOUND`(404), `IR_SCHEMA_INVALID`(422; event 0건 등) |
