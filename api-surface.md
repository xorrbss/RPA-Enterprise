...
| Method | Path | 요청 요지 | 응답 요지 | 주요 ErrorCode |
|---|---|---|---|---|
| GET | `/v1/scenario-generations/capabilities` | `scenario.read` 권한. runtime visual evidence capability 조회 | 200 + `{ visual_evidence: { screenshot: { enabled, policies, default_policy }, video: { enabled, policies, default_policy, artifact_type, media_type } } }`. `video.enabled=false`이면 `video.policies=["never"]`, `default_policy="never"`; true이면 `policies=["never","failure","always"]`, `default_policy="always"` | `AUTHZ_FORBIDDEN`(403) |
