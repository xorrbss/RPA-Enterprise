...
- RBAC 역할 레지스트리·권한 매트릭스의 **본문 정의는 Phase 2**(README §외부 의존 맵 §4). 본 문서는 각 엔드포인트의 권한 게이트 *지점*만 표기하고(예: artifact 조회 RBAC, site risk 승인 권한), 역할명 자체는 추정하지 않는다 — Phase 2 RBAC 확정 전 해당 게이트 구현 착수 시 `TODO: [BLOCKED]`.
...
- **부작용이 있는 명령형 POST**(run create, scenario promote, human-task resolve/assign/escalate, workitem replay, sites approve)에 `Idempotency-Key` 헤더 규약 적용.
...
- run create 시 `params.as_of`(ISO-8601 string)를 **서버가 1회 고정**한다: 요청에 명시되면 그 값을, 미지정이면 서버가 생성 시각으로 채워 `runs.as_of`에 영속화한다(컬럼 위치는 Phase 2 DDL, README §외부 의존 맵 §1).
...
|---|---|---|---|---|
| POST | `/v1/runs` | `Idempotency-Key` 헤더. body: `scenario_version_id`, `params`(params_schema 준수), optional `params.as_of`, optional `workitem_id` | 201 + run 리소스(`run_id`, `status=queued`). `run.created` 이벤트 emit | `IR_SCHEMA_INVALID`(422), `IR_EXPRESSION_COMPILE_ERROR`(422), `SITE_PROFILE_BLOCKED`(403) |
...
- `queued`/`claimed` 단계 abort는 run.started 이전이라 Run 전이가 아니라 dispatcher의 큐/claim 회수로 처리(state-machine §1 "abort 보편성" 주석). API는 동일하게 202를 수락하되 결과는 `cancelled`로 수렴.
...
| POST | `/v1/human-tasks/{human_task_id}/resolve` | `Idempotency-Key`. body: 해소 결과(kind별 payload) | 200 + `resolved`. `human_task.resolved` 이벤트 → Run `resume_requested`(R13/H3) | `HUMAN_TASK_EXPIRED`(410) |
| POST | `/v1/human-tasks/{human_task_id}/assign` | `Idempotency-Key`. body: `assignee` | 200 + `assigned`(H1/H6) | `HUMAN_TASK_EXPIRED`(410) |
| POST | `/v1/human-tasks/{human_task_id}/escalate` | `Idempotency-Key`. body: optional `reason` | 200 + `escalated`(H5). 관리자 큐, Run R15(suspended 유지) | `HUMAN_TASK_EXPIRED`(410) |
...
| PUT | `/v1/gateway/policy` | `If-Match`(정책 버전) + `Idempotency-Key`. body: 정책 갱신 | 200 + 갱신 정책 | `LLM_CAPABILITY_MISMATCH`(422)³ |
...
- 정책의 ETag 대상(버전 컬럼)은 Phase 2 DDL — scenario_versions와 달리 본 패키지에 미정의이므로 If-Match 적용은 구현 시 정책 엔티티 버전 확정 후(notes 참조).
...
- circuit 상태(`site.circuit_opened`/`site.circuit_closed` 이벤트, `SITE_CIRCUIT_OPEN` 503)는 조회로만 노출 — circuit 임계/재개는 운영 정책(README §6), API는 강제 재개를 노출하지 않는다(Phase 2 결정).
- `risk` 등급값(red/amber/green)·site_profiles 컬럼은 Phase 2 DDL(README §외부 의존 맵 §1) — 본 문서는 승인 *흐름*과 에러코드만 고정.
...
- RBAC 역할·권한 매트릭스: `auth-rbac.md`(Phase 2 해소). gateway policy 버전 컬럼은 Phase 2 DDL 미정 → If-Match 적용은 그 후. 전체 OpenAPI 본문(스키마/파라미터/details)은 D1 codegen 위임.
