...
- capability 조회는 콘솔의 기본 evidence 선택을 돕는 read-only 힌트이며, 서버도 `POST /v1/scenario-generations`에서 evidence 생략 시 같은 기본값을 적용한다. 서버는 여전히 `save_and_run`마다 `scenarioGenerationCapabilities.videoRecording`을 최종 권위로 재검증하며, `video!=never`와 capability mismatch는 `video_recording_port_not_configured` blocker로 닫는다.
