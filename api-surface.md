...
월간 자동화 성과 리포트는 `runs`, `run_reruns`, `automation_ideas`, `roi_estimates`, `stagehand_calls`를 조합하는 조회 표면이다. 별도 회계 원장이 아니라 PoC/월간 운영 보고의 근거 자료이며, 비용 총액은 `runs.usage_cost`, 모델별 LLM 비용/토큰은 `stagehand_calls`, ROI는 시나리오에 연결된 `approved/build/operate` automation idea의 최신 ROI estimate 합계로 계산한다.

| Method | Path | 요청 | 응답 | 오류 |
|---|---|---|---|---|
| GET | `/v1/reports/automation-performance` | 쿼리: `?month=YYYY-MM`(기본 Asia/Seoul 현재 월). `run.read` 권한 | 200 + `{ month, timezone, period_start, period_end, summary, cost_by_model, failure_top, trends, by_workflow }`. `summary`는 `total_runs`, `completed`, `failed_business`, `failed_system`, `success_rate`, `rerun_count`, `reprocessing_rate`, `estimated_hours_saved`, `estimated_value`, `implementation_effort`, `net_value`, `value_to_cost_ratio`, `payback_months`, `gateway_cost`, `cost_by_status`, `failed_cost`, `rerun_cost`, `avg_cost_per_run`, `cost_per_completed_run`, `llm_call_cost`, `run_vs_call_cost_delta`, `roi_idea_count`, `roi_confidence`. `cost_by_model`은 `{ model, calls, input_tokens, output_tokens, cost, cost_share }[]`이며 토큰/비용이 모두 NULL이면 합도 `null`. `failure_top`은 실패 code Top 5. `trends`는 일별 run/실패/재처리/비용 추세와 `cost_by_status`, `rerun_cost`, `avg_cost_per_run`, `cost_per_completed_run`, `cost_delta_from_previous_day`. `by_workflow`는 scenario별 run/성공률/재처리율/ROI/비용과 `implementation_effort`, `net_value`, `value_to_cost_ratio`, `payback_months`, `cost_by_status`, `rerun_cost`, `avg_cost_per_run`, `cost_per_completed_run`, `roi_idea_count`, `roi_confidence` | `IR_SCHEMA_INVALID`(422), `AUTHZ_FORBIDDEN`(403) |
| GET | `/v1/reports/automation-performance/export` | 쿼리: `?month=YYYY-MM&format=csv\|xlsx\|poc_markdown`. `format` 생략 시 csv. `run.read` 권한 | csv는 `text/csv; charset=utf-8` + summary/failure/model-cost/trends/workflow 섹션. xlsx는 `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` + Summary/Failure Top N/Cost By Model/Daily Trends/Workflow ROI 시트. `poc_markdown`은 `text/markdown; charset=utf-8` + month, summary metrics, failure Top N, model cost, daily trends, workflow ROI/cost, decision guide를 포함하는 PoC 보고서 템플릿. CSV/XLSX 문자열 셀은 spreadsheet formula injection 방어를 적용하고, Markdown 문자열 셀은 formula 선두 문자, Markdown 링크, HTML 태그 해석을 방어적으로 escape한다. Secret/token/password/resolved secret material은 포함하지 않는다 | `IR_SCHEMA_INVALID`(422), `AUTHZ_FORBIDDEN`(403) |

계산 규칙: `success_rate = completed/(completed+failed_business+failed_system)`, `reprocessing_rate = rerun_count/total_runs`, `net_value = estimated_value - gateway_cost`, `value_to_cost_ratio = estimated_value/gateway_cost`, `payback_months = implementation_effort/estimated_value`, `avg_cost_per_run = gateway_cost/total_runs`, `cost_per_completed_run = completed_cost/completed`, `run_vs_call_cost_delta = gateway_cost - llm_call_cost`. 각 분모가 0이거나 LLM 비용 합계가 전부 NULL이면 `null`로 반환해 0%/100%/0원 차이를 추측하지 않는다. 월 경계는 Asia/Seoul 월초 00:00 이상, 다음 월초 00:00 미만이다. PoC 보고서 템플릿은 동일한 월간 집계만 사용하며, 확대/보류 판단은 `success_rate`, `reprocessing_rate`, run evidence, `net_value > 0`의 설명 가능한 guide로만 제시한다.
...
Product Open v1의 P1 notification channel은 **콘솔용 계산형 알림 센터**다. 콘솔 delivery/ack 원장은 v1에 포함하지만, Teams/Slack/메일/webhook 같은 외부 발송 채널, 수신자 라우팅, snooze, 외부 전송 재시도/DLQ는 V2 P1 범위가 아니며 API가 성공으로 응답하지 않는다. 외부 발송을 추가하려면 SecretRef/connector/notification_deliveries 계약을 별도 버전으로 연다.

v1.1 external delivery 준비 계약(문서화 전용, v1 성공 경로 아님):
- 후보 `OpsAlert.delivery.channel` 값은 `console`(v1 활성)과 future external `teams|slack|email|webhook`이다. 버전된 스키마/API가 열리기 전 외부 채널 생성·전송 요청은 unsupported로 거부해야 하며 `delivered`로 수락하지 않는다.
- future delivery profile은 SecretRef-backed만 허용한다: `channel`, `endpoint_secret_ref`, optional `credential_secret_ref`, non-secret `display_name`, `route_policy_ref`, `allowed_domain_alias` 같은 참조/메타만 API 표면에 둔다. Teams/Slack/email/webhook URL, token, SMTP password, signing secret, webhook path/query 원문은 요청·응답·감사 payload 필드가 될 수 없다.
- future `notification_deliveries` 원장은 `ops_alert_acknowledgements`와 독립이다. `POST /ack`는 알림 acknowledgement visibility만 바꾸며, 외부 전송 성공 증거·retry 취소·receipt 상태 변경으로 해석하지 않는다. 반대로 외부 전송 실패는 ack를 실패시키지 않는다.
- fail-closed/deferred rule: SecretRef resolve, connector enablement, route policy, allowlist, runtime sender, provider receipt 중 하나라도 없거나 불확실하면 external leg는 `deferred` 또는 미인큐로 남기고, API는 가짜 성공을 반환하지 않는다. future `sent`/`delivered` status는 PlainSecret/resolved endpoint를 포함하지 않는 provider receipt metadata가 있을 때만 가능하다.
- v1.1 구현 전 owner input: 채택 channel set, SecretRef namespace/backend alias, notification sender runtime identity, recipient/group routing owner, retry/DLQ 한도, 허용 provider/domain, 운영 rotation/break-glass owner. Product Open v1은 console-only라 이 input은 release checklist blocker가 아니다.

| Method | Path | 요청 요지 | 응답 요지 | 주요 ErrorCode |
|---|---|---|---|---|
| GET | `/v1/ops-alerts` | query: optional `severity`(`critical`/`warning`/`info`), optional `source`(`run_sla`/`human_task_sla`/`trigger_fire`/`failure_spike`/`dlq`/`bot_pool`), optional `status`(`open` 기본/`acknowledged`/`all`), `limit`. `ops_alert.read` 권한 | 200 + `{ items, next_cursor:null }`, 각 item은 `OpsAlert`(`alert_id`, `severity`, `source`, `title`, `detail`, `subject_type`, `subject_id`, `status`, `delivery`, `ack`, `recommended_action`, `route`, `detected_at`, optional `due_at`). `delivery`는 v1에서 console-only(`external_delivery:false`) | `AUTHZ_FORBIDDEN`(403), `IR_SCHEMA_INVALID`(422) |
...
Bot pool(v1)은 운영 용량 표면용 read model이다. `capacity_slots`는 호출 테넌트의 assigned pool 기준으로 최근 heartbeat가 있고 circuit이 닫힌 active browser worker 수를 계산한다. assigned pool이 없으면 membership이 없는 default worker만 기본 capacity로 본다. 2분 이상 heartbeat가 없는 worker는 `workers.stale`로 집계하고 용량 슬롯에서는 제외한다. `leases`와 `queue`는 요청 테넌트 범위로만 계산하며, 만료된 reserved/active browser lease가 있으면 `health=critical`이다. 저장/수정 가능한 SLA 라우팅, 외부 알림 발송은 별도 계약이 열릴 때까지 v1 범위가 아니다.
